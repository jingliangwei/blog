...
- X 射线源统计分类： Color-Color 图
    $$
    C_{MS}=\frac{M-S}{H+M+S}
    $$
    $$
    C_{HM}=\frac{H-M}{H+M+S}
    $$
    Soft: 0.3-1 keV
    ...
    ![c-c](./high_energy_astrophysics_fig/6.png)
...
- 射电星系：
    ...
    Fanaroff-Riley Class (FRI, FRII)

- 类星体 Quasar:

    能量更高，只能看到活动星系核，看不到星系

- 耀变体 Blazars:

    快速光变（小时），无显著谱线

同一模型：一样的活动星系核，观测角度不同

![AGN](./high_energy_astrophysics_fig/7.png)
...
- 反响映射：
    ...
    通过发射线对连续谱变化的响应，得到宽线区尺度和结构信息。

    时间延迟 -> 宽线区气体云块到中心黑洞的距离

    谱线展宽 -> 气体云块的运动速度

    中心黑洞质量 $M\sim\dfrac{rv^2}{G}$

#### 射电噪 AGN 的高能辐射

- 射电噪 AGN 的射电辐射是一般星系的 $10^8$ 倍
...
- 视超光速运动
    ...
    $$
    v_\text{app}=\frac{\Delta l}{\Delta t}=\frac{v\sin\phi}{1-\frac{v}{c}\cos\phi}
    $$
...
- 多普勒增亮
    ...
    I_{\nu_\text{obs}}=D^3I_{\nu_\text{em}}
    $$
    多普勒因子 $D=\dfrac{1}{\gamma(1-\beta\cos\theta)}$
